...
const noFlashScript = `(function(){try{var r=document.documentElement;var t=localStorage.getItem('pui-theme');if(t==='light'||t==='dark'){r.setAttribute('data-theme',t);}var s=JSON.parse(localStorage.getItem('pui-studio')||'null');if(s&&s.root){for(var k in s.root.vars){r.style.setProperty(k,s.root.vars[k]);}for(var d in s.root.data){r.setAttribute('data-pui-'+d,s.root.data[d]);}}}catch(e){}})();`;
