...
or with the library's own switch. [`ThemeToggle`](/components/theme-toggle/) goes anywhere; parking it in a corner is what `create-forte-ui` scaffolds, so `src/App.tsx` from step 5 becomes:
