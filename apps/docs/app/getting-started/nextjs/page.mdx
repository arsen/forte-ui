...
Every component ships with `"use client"` already in place, so your pages and layouts stay server components and just import from `@forte-ui/react` — no provider, no registry, no configuration file.
...
<Callout kind="note">
  Nothing above is Next.js-specific beyond the file names — the same two (or
  three) CSS imports work in any bundler that handles CSS imports. A dedicated
  Vite walkthrough is planned.
</Callout>
