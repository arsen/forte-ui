...
`Alert.Close` dismisses the card on its own, so the common case needs no state and no handler at all. The card does not disappear on the click — it collapses out of the flow over `--pui-alert-exit-duration`, and the content below rises into the space rather than jumping up once the element is gone.
...
Children of `Alert.Icon` replace the tone's standard glyph rather than sitting beside it, and an `svg` is sized to `--pui-alert-icon-size` without a width of its own. Keep it `aria-hidden` — the alert's text already says what happened.
...
`Alert.Close` is a 24×24 button (SC 2.5.8) with an `aria-label` of "Dismiss", a `.pui-focus-ring`, and `type="button"` — the last because the HTML default is `submit`, and an alert inside a form is exactly where that bites.
...
The five `--pui-alert-tone-*` slots are the whole colour system: `tone` fills them in, and `variant` decides which of them reach the surface. A one-off tone is those five properties on one element, with no new variant and no wrapper.
...
<Callout>
  The status families gained `--pui-color-success-soft`, `--pui-color-success-border` and their `warning` and `info` counterparts for this component, so all four status tones now offer the same set `danger` always did. They are ordinary tokens: anything can use them.
</Callout>
