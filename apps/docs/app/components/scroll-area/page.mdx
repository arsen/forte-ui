...
A tab strip is the case that most wants this component: the tabs are fixed-width, they cannot wrap, and there is no room to show all of them. Wrap `Tabs.List` — not `Tabs.Root` — in a scroll area, so the strip scrolls and the panels below it stay put.
...
- **Arrowing to an off-screen tab scrolls it into view, clear of the fade.** The browser brings a newly focused tab into view on its own; the `scroll-margin` that every `.pui-focus-ring` element carries is what stops it landing right against the edge, half-faded, with its focus ring underneath the gradient.

<Callout kind="a11y">
  A scrollable strip adds one tab stop *before* the tabs — the viewport itself, which Base UI makes focusable so that a keyboard user can scroll a region that has no other controls in it. That is the correct behaviour, not a stray stop; SC 2.1.1 requires it. It disappears on its own when the strip fits.
</Callout>
...
.viewport {
  mask-image: linear-gradient(
    to bottom,
    transparent 0,
    black min(var(--pui-scroll-area-fade-size), var(--scroll-area-overflow-y-start, 0px)),
...
There is no `prefers-reduced-motion` block in the stylesheet. The only motion in the component is the scrollbars fading in and out, which runs on `--pui-duration-fast` and collapses with it; the fade itself is not motion, and scrolling is the user's own.
...
| :-- | :-- | :-- |
| `--pui-scroll-area-fade-size` | How far the fade reaches in from an edge, at most | `var(--pui-space-5)` |
| `--pui-scroll-area-scrollbar-size` | Track thickness on both axes | `var(--pui-space-3)` |
| `--pui-scroll-area-track-padding` | Inset between the track's edge and its thumb | `2px` |
| `--pui-scroll-area-track-bg` | Track fill | `transparent` |
| `--pui-scroll-area-track-bg-hover` | Track fill while the pointer is over it | `var(--pui-color-panel)` |
| `--pui-scroll-area-thumb-bg` | Thumb fill | `var(--pui-color-border-strong)` |
| `--pui-scroll-area-thumb-bg-hover` | Thumb fill on hover | `var(--pui-color-foreground-subtle)` |
| `--pui-scroll-area-thumb-bg-active` | Thumb fill while being dragged | `var(--pui-color-foreground-muted)` |
| `--pui-scroll-area-thumb-radius` | Corner radius of the track and thumb | `var(--pui-radius-pill)` |
| `--pui-scroll-area-corner-bg` | Fill of the square between the two scrollbars | `transparent` |
