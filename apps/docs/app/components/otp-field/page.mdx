...
the side comes from the same `--pui-control-*` tokens `Input` and
`Select.Trigger` read — so a code field and the rest of a form stay on one
scale at every `data-pui-density` setting.
...
rather than being rejected by a full slot. That is also why the selection colour
...
next to the caret they are watching. What does transition is colour: the
boundary on hover, on completion, and on failure.

## Theming

Every `--pui-otp-field-*` property is declared on the field's root and reaches
...
`--pui-otp-field-gap` is deliberately not `--pui-control-gap`. That token drops
to 4px under `data-pui-density="compact"`, and 4px is less than a focus ring
...
its props, so it carries `data-pui="separator"` rather than a marker of its own —
scope to it with `[data-pui="otp-field"] [data-pui="separator"]`.
