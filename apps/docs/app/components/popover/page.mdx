...
```tsx
import { Popover } from "@dofortech/pretty-ui";
```
...
`--pui-popover-z-index` is declared on the positioner and
`--pui-popover-backdrop-z-index` on the backdrop; the viewport pair is declared
...
resolve to — `--pui-color-overlay`, `--pui-color-border`,
`--pui-radius-surface`, `--pui-surface-p`, `--pui-shadow-4`, `--pui-space-*`,
`--pui-font-*`, `--pui-travel-sm`. The popup only reads those and never
re-declares them, so a value set on `:root` or a theme scope does reach it. The
knobs with literal defaults — `--pui-popover-max-width`,
`--pui-popover-border-width`, `--pui-popover-arrow-width`,
`--pui-popover-arrow-height` and the two z-indexes — have no global token behind
...
photo. `--pui-popover-border-color: transparent` gives you the shadow-only look
...
The stylesheet also computes `--pui-popover-enter-x` / `--pui-popover-enter-y`
from `--pui-popover-travel` and the resolved `data-side`, and
`--pui-popover-viewport-x` from `--pui-popover-viewport-travel` and
