...
Three sizes, set once on the root. Cell size follows `data-pui-density` too, so a compact app gets a compact `md` for free.
...
One knob is worth knowing before the rest: `--pui-calendar-cell-size` is the measure everything else derives from. The arrows, the caption row's height and the column widths all read it, so resizing a calendar past the three `size` presets is one value, not six.
...
Both navigation arrows are `Button`s, so they answer to `navVariant` and to every `--pui-button-*` knob set on the calendar. They are told apart by `data-nav="previous"` / `data-nav="next"`.
