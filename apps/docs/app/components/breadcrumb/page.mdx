...
The default chevron mirrors itself in RTL. A separator you pass does **not** — mirroring a glyph would turn a `/` into a `\`, so only the built-in icon consults `--pui-direction`. Flip the demo to RTL with the frame's direction control to see both halves of that.
...
- **Crumbs meet the 24×24 target floor** (SC 2.5.8) through `.pui-target`, which grows the hit box without moving anything — so the `plain` variant can keep the zero padding its look depends on.
