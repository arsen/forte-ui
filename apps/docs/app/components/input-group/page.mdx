...
height, and wearing `.pui-target` so the hit area stays 24×24 even where the
...
the control and every addon at once — on the same `--pui-control-*` scale the
...
Every `--pui-input-group-*` property is declared on the group element itself,
...
`@layer pretty-ui.components` at any specificity) or an inline `style`. The
global tokens they resolve to — `--pui-color-*`, `--pui-control-*`,
`--pui-radius-*` — are what to re-point from `:root` or a theme scope to move
every group at once.
