...
`--pui-navigation-menu-column-width`.
...
— and it is capped again by `--pui-navigation-menu-content-inset` from the
...
mirrors under RTL through `--pui-direction`.
...
`--pui-navigation-menu-bridge`, paired with the popup's `sideOffset` — so
...
cannot shave it off (SC 2.4.11). Rows are floored at `--pui-target-min` so a
...
every other row. The popup carries `.pui-hc-surface`, whose transparent border
...
`--pui-duration-move`. `[data-instant]` drops the transition for the changes
...
distance is **not** multiplied by `--pui-direction`. The usual rule assumes the
...
collapse their own geometry: `--pui-travel-sm` and `--pui-travel-lg` become
`0px`, `--pui-scale-enter` becomes exactly `1`, and `--pui-duration-move`
...
column instead — `--pui-color-*`, `--pui-radius-*`, `--pui-space-*`,
`--pui-control-*`, `--pui-font-*` and the motion tokens are only read here,
...
the tables below list what pretty-ui adds or changes the default of. Every part
