...
The popup's surface is **not** restyled by this component. It is a `Popover.Popup`, keeps `data-pui="popover-popup"` and every `--pui-popover-*` knob, and the only thing the picker does to it is step the surface padding down — a calendar arrives with its own measures, and a full `--pui-surface-p` around them reads as a frame rather than a margin. The column inside it is this component's own part, `data-pui="date-picker-panel"`, which is what you target to reach *this* popup rather than every popover on the page.

`DatePicker.Clear` is a `Button` and tags itself `data-pui="button"`, the way every composed component in the library does — scope it as `[data-pui="date-picker-footer"] [data-pui="button"]`.
