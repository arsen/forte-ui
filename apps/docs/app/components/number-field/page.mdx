...
```tsx
import { NumberField } from "@dofortech/pretty-ui";
```
...
`--pui-number-field-scrub-grip-opacity` (0.7), so it reads as an affordance
...
`--pui-control-*` tokens `Input` and `Select.Trigger` read — so all three line up
on one row at every `data-pui-density` setting. The steppers stay square because
...
The steppers also carry `.pui-target`, because at `sm` under
`data-pui-density="compact"` the button inside the group's border is 22px —
...
<Callout kind="warn">
  ...
  Everything the library ships lives in `@layer pretty-ui.*`, which an app's own
...
:focus-visible:not(:where(.pui-focus-ring-within *)) {
  outline: var(--pui-focus-ring-width) solid var(--pui-focus-ring-inner);
  outline-offset: var(--pui-focus-ring-offset);
  box-shadow: 0 0 0 var(--pui-focus-ring-offset) var(--pui-focus-ring-outer);
}
```
...
part, through `className` (an unlayered rule beats `@layer pretty-ui.components`
...
<Callout kind="note">
  `--pui-number-field-scrub-cursor-*` is set on an element that is portalled to
  `<body>`, which means it does **not** inherit a `.pui-theme` scope — the same
