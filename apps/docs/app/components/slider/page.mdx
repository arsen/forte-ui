...
- `edge` insets the thumb so its outer edge lands on the control's edge instead. Base UI measures the control's border and padding to work this out, which means it re-measures if you change `--pui-slider-control-padding`.
...
`orientation="vertical"` turns the same parts 90°. The root becomes a single centred column and `--pui-slider-length` now describes the block axis, defaulting to `10rem`.
...
The thumb's ring is `--pui-color-primary` and the rail is `--pui-color-border-muted`. The value is legible from three independent places — the indicator's length against the rail, the thumb's position, and the `Slider.Value` text — so no single pair is load-bearing on its own. If your accent seed lands pale against your page, re-point `--pui-slider-thumb-border` at `--pui-color-foreground-subtle`, which is measured at 3.73:1.
...
Base UI rewrites the indicator's `width` and the thumb's `inset-inline-start` on every value change — which during a drag means every `pointermove`. A positional transition therefore does not run once, it *restarts every frame*, leaving the handle permanently interpolating toward a target that has already moved. It trails the pointer, and it keeps a layout-triggering property under animation for the entire gesture. An earlier version of this component had a `--pui-duration-move` glide with the transition suppressed during a drag; both suppression triggers turned out to be unreliable — `:active` is cleared the moment the pointer leaves the control's box, and `[data-dragging]` is React state that only arrives after Base UI's intentional-drag threshold — so in practice the suppression never fired at all.
...
Every property below is declared on `Slider.Root` and consumed by the parts underneath through inheritance. Setting one on an ancestor of the root has no effect — the root's own declaration wins over an inherited value. Override them on the root itself, through `className` (an unlayered rule beats the library's `@layer pretty-ui.components`, whatever its specificity) or an inline `style`.

| Property | Controls | Default |
| --- | --- | --- |
| `--pui-slider-length` | Control length: `inline-size` when horizontal, `block-size` when vertical. Capped at `100%` of the container. | `16rem` (`10rem` when vertical) |
| `--pui-slider-track-thickness` | Rail thickness, and one of the two inputs the control padding derives from. | `4px` (`sm` `3px`, `lg` `6px`) |
| `--pui-slider-thumb-size` | Thumb width and height. | `1rem` (`sm` `0.875rem`, `lg` `1.25rem`) |
| `--pui-slider-thumb-border-width` | Thickness of the thumb's ring. | `2px` |
| `--pui-slider-radius` | Rail and indicator corner radius. | `var(--pui-radius-pill)` |
| `--pui-slider-gap` | Gap between the label/value row and the control. | `var(--pui-space-2)` |
| `--pui-slider-track-bg` | Rail fill — the empty part of the range. | `var(--pui-color-border-muted)` |
| `--pui-slider-indicator-bg` | Fill of the selected part. | `var(--pui-color-primary)`, per `tone` |
| `--pui-slider-thumb-bg` | Thumb fill. | `var(--pui-color-background)` |
| `--pui-slider-thumb-border` | Thumb ring colour. | `var(--pui-color-primary)`, per `tone`, `var(--pui-color-danger-border)` when invalid inside a field |
| `--pui-slider-thumb-shadow` | Thumb shadow. | `var(--pui-shadow-1)` |
| `--pui-slider-thumb-halo-color` | Base colour the halo is mixed from. | `var(--pui-color-primary)`, per `tone` |
| `--pui-slider-thumb-halo-strength` | How much of that colour survives the mix toward `transparent`. | `48%` (`neutral` `30%`) |
| `--pui-slider-thumb-halo` | The resolved halo colour. Derived from the two above; set it directly to bypass them. | `color-mix(in oklab, <halo-color> <strength>, transparent)` |
| `--pui-slider-thumb-halo-size` | Current halo radius. Set by hover and drag; set it yourself for a halo that is always on. | `0px` |
| `--pui-slider-thumb-halo-size-active` | Halo radius in those states. It stands down while the focus ring is up, so the ring's outer tone is not covered. | `5px` (`sm` `4px`, `lg` `6px`) |
| `--pui-slider-control-padding` | How far the control is padded past the rail — the SC 2.5.8 target. | `max(24px − thickness, thumb − thickness + 4px) ÷ 2` |

`--pui-slider-control-padding` is derived from the rail thickness and the thumb size, so resizing either keeps the pointer target correct — you rarely want to set it by hand. Note that `thumbAlignment="edge"` measures it, so changing it also moves where an edge-aligned thumb rests.
