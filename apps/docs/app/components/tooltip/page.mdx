...
icon is the usual case — it is a [popover](/components/popover), not a tooltip,
...
convenience `<Menu.Shortcut>` is on the [menu](/components/menu) side, for the
toolbar button that runs the same command. Write the keys the way they are
...
<DemoBlock id="tooltip/shortcut" />

<Callout kind="a11y">

**Put the shortcut on the trigger as `aria-keyshortcuts` as well.** Unlike
`Menu.Shortcut`, this part is not `aria-hidden` — there is nothing to hide it
from, since no part of a tooltip is announced. Which is the problem:
`aria-keyshortcuts="Meta+B"` on the trigger is the only route the keys have to
a screen reader user, and it wants them spelled in words. Read aloud, `⌘B`
becomes "place of interest sign B".
...
`--pui-tooltip-shortcut-bg` and the three knobs beside it, which is also how a
...
knobs with literal defaults — `--pui-tooltip-max-width`,
`--pui-tooltip-arrow-width`, `--pui-tooltip-arrow-height`, and the shortcut
cap's `--pui-tooltip-shortcut-bg` and `--pui-tooltip-shortcut-border-color`,
...
<PropTable component="TooltipShortcut" />
