...
The difference is not decorative. A drawer's edge is what makes the drag gesture legible: it comes from somewhere and it goes back there, so pulling it that way to dismiss it is a guess anyone can make. A centered dialog has no such direction, which is why it has no such gesture.
...
Neither behavior needs a `prefers-reduced-motion` query in this component, and there isn't one.
...
`Root` renders no DOM element, so it takes no `className`, `style` or `ref` — style the popup instead. It forwards every [Base UI root prop](https://base-ui.com/react/components/drawer) unchanged: `open`, `defaultOpen`, `onOpenChange`, `onOpenChangeComplete`, `modal`, `disablePointerDismissal`, `actionsRef`, `handle`, `triggerId`, `defaultTriggerId`, `snapPoint`, `defaultSnapPoint`, `onSnapPointChange` and `snapToSequentialPoints`. The one it does not forward is `swipeDirection`, which `side` replaces — the two are the same fact stated twice, and a component that accepted both would let you contradict yourself.
