...
margin and `<a>` its color, because a box that quietly stripped the element it
...
- **Contrast for anything laid over media.** Text on a photograph has no
  guaranteed background, so SC 1.4.3 cannot be met by the text color alone.
  Put it on a band or a scrim, as the overlay example does, rather than trusting
  the image to stay dark where the words are.

<Callout kind="a11y">
  Under forced colors, `filled`'s background is replaced by a system color and
  ...
  `outlined` needs nothing: border colors survive forced colors, which is why
...
open or closed — so there is nothing to animate, and a color transition on the
