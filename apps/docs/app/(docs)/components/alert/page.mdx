...
`variant` is how loud the card is; `tone` is which color set it draws from. Two variants rather than the usual three: there is no `solid`, because a solid fill puts body copy on a saturated field, and the paragraph — not the label — is the part an alert exists to have read.

<DemoBlock id="alert/variants" />

`soft` tints the whole surface, its edge and its type in the tone. `outline` is a neutral panel with a hairline, where the tone survives only in the glyph — the arrangement `Toast` uses, and for the same reason: several alerts on one screen have to stay distinguishable from each other, and a page of tinted cards has nothing left to emphasize with. Reach for `outline` when the alert sits inside a form beside other bordered controls.
...
It sits against the inline-end edge, centered on the whole message rather than on the title, so a three-line description does not leave the button floating at the top. Two controls fit as well — they share a flex row, so the pair reads as one decision.
...
The interesting decision here is not the color. It is whether the alert should announce itself, and the answer is usually **no**.
...
Color is never the only cue. The four status glyphs are four different **shapes** — a tick, a cross, a triangle, an "i" — rather than one shape in four colors, which is what keeps the kind of message readable to a color-blind reader and under forced colors, where every glyph repaints in a single system color (SC 1.4.1). The title says it in words as well.

Every text pair the component can produce clears AA, and the harness now asserts it. `soft` paints its message in the tone's step 11 on the tone's step 3 — `danger-11 on -3`, `success-11 on -3`, `warning-11 on -3`, `info-11 on -3` in both light and dark, plus `gray-11 on gray-3` for `neutral`, which is swept across the same ~119k seeds as the rest of the ramp. That is why the title and the description are the *same color* on a colored tone, with weight doing the separating: a second, lighter tint of the same hue is not a color the ramp promises anything about.
...
Under forced colors the card keeps its border and loses its tint: backgrounds are replaced with system colors, so `soft` and `outline` become the same card and the tone stops being visible at all. The glyph shapes carry it from there.
...
The one other transition in the file is the close button's hover, on color alone.
...
The five `--forte-alert-tone-*` slots are the whole color system: `tone` fills them in, and `variant` decides which of them reach the surface. A one-off tone is those five properties on one element, with no new variant and no wrapper.
