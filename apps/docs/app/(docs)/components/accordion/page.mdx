...
Each `Accordion.Header` renders an `<h3>` carrying a `<button>` with `aria-expanded` and an `aria-controls` pointing at its panel; the panel is a `role="region"` labeled by its trigger. `<h3>` is right only if the accordion sits under an `<h2>` — pass `render={<h2 />}` (or whichever level fits) to keep the outline ordered:
...
Under forced colors both dividing lines are pinned to `CanvasText`, since a card whose border was replaced by a system color is the only thing still holding the shape once shadows and backgrounds are stripped. Nothing paints `Highlight`: an open row is not a selected row, and the chevron's rotation is geometry, so it survives that mode untouched and goes on saying which rows are open. Disabled items are painted `GrayText` rather than relying on opacity, which forced colors does not honor.
...
There is no `prefers-reduced-motion` block in the accordion stylesheet, and none anywhere else in a component file. The tokens do it: `--forte-duration-normal` collapses from 240ms to 120ms and `--forte-travel-sm` to `0px`, so the panel snaps open and the content fades in place without traveling. Durations shorten rather than reaching `0s`, so every transition still fires.
