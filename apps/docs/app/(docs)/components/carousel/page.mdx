...
Under forced colors the controls keep a system-colored border in place of their shadow, the active dot is painted `Highlight`, and the active thumbnail's ring is `Highlight` while the inactive ones' become invisible — `transparent` is replaced in that mode, so they are set to `Canvas`, the page color, instead.
