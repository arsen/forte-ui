...
the cue is not color alone. In a real app it comes from the router —
...
<KeyboardTable
  rows={[
    { keys: ["Tab"], description: "Moves focus through the items in the bar, and then out of it. From the trigger of an open panel it moves into that panel instead." },
    { keys: ["Arrow Right", "Arrow Left"], description: "Horizontal orientation: moves between items in the bar, skipping disabled triggers. Mirrored under RTL." },
    { keys: ["Arrow Down", "Arrow Up"], description: "Vertical orientation: moves between items in the bar. With a horizontal bar, Arrow Down opens the focused trigger's panel and leaves focus on the trigger." },
    { keys: ["Enter", "Space"], description: "Opens the focused trigger's panel, or follows the focused link. Focus stays on the trigger — Tab is what moves into the panel." },
...
becomes a system-colored boundary once the shadow is stripped, and the arrow is
two flat SVG paths rather than the traditional CSS-border triangle — a border
triangle degrades into a filled rectangle when every border color is forced to
`CanvasText`.
...
Each part forwards every prop it does not consume to its [Base UI counterpart](https://base-ui.com/react/components/navigation-menu), so
