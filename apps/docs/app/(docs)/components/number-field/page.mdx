...
```
  ...
  with it because the element holding its focus is visually hidden. The other
  three exclusions are the same statement made in the library's own markup:
  `[data-focus-inset]` is a part that rings *inward* because a scroll container
  would crop an outset ring, and `.forte-focus-ring-none` (with the
  `combobox-input`, which predates it) is a part that takes focus and must not
  ring at all — a `Menu` or `Select` popup, which Base UI focuses as its focus
  trap's entry point. Drop an exclusion and that part wears a ring the
  component deliberately left off.
</Callout>
...
The tables list what each part adds on top of [Base UI](https://base-ui.com/react/components/number-field). Everything else passes
straight through.
