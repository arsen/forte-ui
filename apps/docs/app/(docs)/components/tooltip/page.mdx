...
For transient feedback such as "Copied!", reach for [`Toast`](/components/toast/)
instead — unlike a tooltip, it is announced.
...
[`Kbd`](/components/kbd/), re-tuned through the tooltip's own knobs, so it
...
The cap keeps the popup's own text color and carries a translucent fill drawn
from it, rather than dimming the way a menu's shortcut does. A tooltip's
surface is the inverse of the page — near-white on near-black — and no mid-gray
...
drawn from all three. Everything else passes through to the
[Base UI parts](https://base-ui.com/react/components/tooltip) underneath.
