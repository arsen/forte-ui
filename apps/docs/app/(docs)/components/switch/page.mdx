...
3.73:1 against the page — above the 3:1 that SC 1.4.11 asks of non-text controls. In forced-colors
...
automatic system colors from the UA. The disabled state overrides its own opacity there and uses
`GrayText` instead, since forced colors does not touch `opacity` and a 55% control would otherwise
...
with no visible on/off difference except color. So the travel is never multiplied by
...
`stateIcon={false}` removes that cue and leaves color as the only non-positional one; the state
...
The table lists the props `Switch` adds. Everything else on [Base UI's `Switch.Root`](https://base-ui.com/react/components/switch) passes straight
