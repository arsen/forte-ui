...
Four levels of chrome. `plain` is a bare row; `panel` fills with the panel color; `outline` sits on the page background with a hairline underneath; `frosted` is `outline` made translucent, with the content behind it blurred.
...
`tone` is the second axis — which color set the bar draws from — and it is independent of `variant`: a `panel` bar in `primary` is filled with the accent, an `outline` one sits on the accent's soft step.

<DemoBlock id="app-bar/tones" />

Nothing inside the bar is told the tone. A colored bar **re-points the foreground tokens for its subtree** — `--forte-color-foreground`, `-foreground-muted`, `-panel-hover`, `-panel-active` and the two border tokens — so the ghost buttons, the muted caption and the separator you compose into it recolor themselves for contrast, without a prop. Popups opened from the bar are portaled to `<body>` and keep the page's palette, which is what you want from a menu.
...
`elevateOnScroll` is the Material "on scroll" behavior: at the top the bar is part of the page, with no fill and no edge, and once content is under it, it is a surface over the page — the variant's fill and hairline, plus a shadow.

<DemoBlock id="app-bar/sticky" />

Only a `neutral` bar drops its fill at rest. On a colored tone the fill is what carries the text's contrast, so there the surface stays and only the shadow is deferred. And `plain` has no surface to reveal, so pair the prop with one of the other three.
...
**Under forced colors** the fill, the blur and the shadow are stripped, and the hairline is the whole bar: every variant paints it in the system ink, `plain` included. Text and icons take `CanvasText`, and `frosted` is opaque there and under `prefers-contrast: more` alike.
