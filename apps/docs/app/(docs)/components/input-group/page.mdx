...
A group squeezed by its container — a flex item in a toolbar, a sidebar
filter — shrinks its control and keeps every inline addon on the same row.
Only a `block-start` or `block-end` addon gives the group a second row.

<DemoBlock id="input-group/narrow" />
...
read-only tints it; invalid recolors the boundary and the caret, and nothing
else — recoloring the value or the addons would make the user's own answer
...
otherwise be replaced with an arbitrary system color. A disabled group also
