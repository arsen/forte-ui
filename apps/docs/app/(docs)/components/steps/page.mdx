...
`labelPlacement="below"` centers each title under its circle and gives every step an equal share of the row. The connector then runs from one circle's center to the next — which is what the equal shares are for. It suits a short wizard whose titles are a word or two; a title that needs a description reads better beside the circle.
...
`tone` colors "done" and "here" and nothing else. A step not yet reached stays neutral in every tone, so the color only ever says "this far" — and the error state is always `danger`, whatever the tone.

<DemoBlock id="steps/tones" />

### Icons

Children of `Steps.Indicator` replace the number, in every state. The fill color still carries the status, and a completed step still gets its visually hidden "Completed" for a screen reader — the icon is not asked to mean "done" on its own.
...
- **State is never color alone.** Active and complete share a fill; what tells them apart is the check, the halo and the title's weight. Under forced colors, where every fill goes to `Canvas`, the reached steps are named `Highlight` and the rest get a `CanvasText` ring so they stay circles at all.
