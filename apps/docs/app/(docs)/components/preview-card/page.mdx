...
every pass, and the root and its triggers stop recognizing each other. `open()`
...
ordinary link and the card never interrupts — which is the correct behavior for
...
color, so a paragraph with six mentions in it does not turn into six rules of
solid accent.
...
[Base UI parts](https://base-ui.com/react/components/preview-card) underneath.
