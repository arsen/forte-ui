...
Every derived colour re-resolves; there is no second stylesheet to load. The same attribute, along with `.forte-theme` / `data-forte-theme` scoping for islands, is covered in [Theming](/customization/theming/), and [`useTheme`](/components/theme-toggle/#usetheme) is the hook under the toggle when you want your own control.
